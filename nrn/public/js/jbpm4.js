...
JBPM = function(obj) {
	this.init.apply(this, [ obj ]);
};
JBPM.DocUtil = new Object();
JBPM.DocUtil.BindAsEventListener = function(object, fun) {
...
JBPM.DocUtil.isFF = !JBPM.DocUtil.isIE;
...
JBPM.DocUtil.appendJScript = function(fileStr) {
	document.write("<script type='text/javascript' src='" + fileStr
			+ "'></script>");
};
...
JBPM.getX = function(event) {
	var temp = event.x;
	if (JBPM.DocUtil.isIE
			&& (JBPM.DocUtil.getIEVersion() == 8 || JBPM.DocUtil.getIEVersion() == 6)) {
		temp = event.clientX + document.documentElement.scrollLeft
				- document.body.clientLeft;
	}
	var x = event.pageX || temp;
	if (!JBPM.DocUtil.isIE
			|| (JBPM.DocUtil.isIE && JBPM.DocUtil.getIEVersion() >= 8)
			|| (JBPM.DocUtil.isIE && JBPM.DocUtil.getIEVersion() == 6)) {
...
JBPM.getY = function(event) {
	...
	if (JBPM.DocUtil.isIE
			&& (JBPM.DocUtil.getIEVersion() == 8 || JBPM.DocUtil.getIEVersion() == 6)) {
		temp = event.clientY + document.documentElement.scrollTop
...
JBPM.console.style.display = 'none';
...
JBPM.console.appendInfo = function(str, type) {
	...
	var s = cd.getHours() + ":" + cd.getMinutes() + ":" + cd.getSeconds() + ' '
			+ dt + ' ' + str;
...
JBPM.window.showInfo = function(htmlStr) {
	...
	JBPM.window.show();
	JBPM.window.container.style.top = parseInt((JBPM.container.offsetHeight - JBPM.window.container.offsetHeight) / 2)
			+ 'px';
};
...
JBPM.prototype = {
	init : function(obj) {
		...
		// ///////////////////////////////////
		JBPM.Image.relative_path = obj.relative_path ? obj.relative_path
				: "images/";
		JBPM.loadImages();
		JBPM.container.style.cssText = obj.cssText ? obj.cssText : "";
		JBPM.container.style.width = obj.width ? obj.width : "100%";
//		JBPM.container.style.height = obj.height ? obj.height : "800px";
		JBPM.container.style.height = "800px";
		JBPM.container.style.position = 'relative';
		JBPM.container.style.border = obj.border ? obj.border
				: 'black 0px solid';
		if (obj.painter == null) {
	...
	initToolBarContainer : function() {
		...
		with (JBPM.toolBarContainer.style) {
			width = '420px';
			height = 'auto';
			left = parseInt(JBPM.toolBox.left + JBPM.toolBox.getWidth() + 10)
					+ 'px';
		...
		td.style.whiteSpace = "nowrap";
		td.align = 'center';
		// td.innerHTML = '<a href="'+JBPM.Text.help+'" target="_blank"
		// style="color:black;text-decoration:none;">'+JBPM.Text.helpBtn+'</a>';
		tr.appendChild(td);
		td = document.createElement('td');
		td.style.width = '50px'
		td.style.whiteSpace = "nowrap";
		...
		about.onclick = function() {
			JBPM.window.showInfo(JBPM.Text.about + "<br>" + JBPM.Text.moreInfo
					+ "<a target='_blank' href='" + JBPM.Text.help + "'>"
					+ JBPM.Text.help + "</a>");
		};
	...
	initPropertiesContainer : function() {
		...
		title.appendChild(imgBtn);
		title.appendChild(document.createTextNode(' 属性'));
		title.style.fontSize = '12px';
...
JBPM.Painter.prototype = {
	init : function(obj) {
		...
		this._clickHandle = JBPM.DocUtil.BindAsEventListener(this,
				this.clickHandler);
		JBPM.DocUtil.addEventHandler(this.canvas, "click", this._clickHandle);
	},
	initPainter : function() {
		var css = this.paramObj.cssStyle ? this.paramObj.cssStyle.replace(
				"position", ";position") : "";
		if (this.paramObj.showGrid != false) {
			css = "background:url(" + JBPM.Image.canvas_bg
					+ ") -9px 9px repeat;" + css;
		}
	...
	initStyle : function(obj) {
		...
		this.arrowLength = obj.arrowLength ? obj.arrowLength
				: 10 * this.context.lineWidth;
	},
	initEvent : function() {
		this._startHandle = JBPM.DocUtil.BindAsEventListener(this,
				this.startMove);
		...
		this._clickHandler = JBPM.DocUtil.BindAsEventListener(this,
				this.showPoints);
		JBPM.DocUtil.addEventHandler(this.canvas, "mousedown",
				this._startHandle);
		JBPM.DocUtil.addEventHandler(this.canvas, "click", this._clickHandler);
		if (JBPM.DocUtil.isFF) {
			JBPM.DocUtil.addEventHandler(document, "mousemove",
					this._moveHandler);
			JBPM.DocUtil.addEventHandler(document, "mouseup", this._endHandler);
		}
		if (JBPM.DocUtil.isIE) {
			JBPM.DocUtil.addEventHandler(this.canvas, "mousemove",
					this._moveHandler);
			JBPM.DocUtil.addEventHandler(this.canvas, "mouseup",
					this._endHandler);
		}
	},
	removeEvent : function() {
		JBPM.DocUtil.removeEventHandler(this.canvas, "mousedown",
				this._startHandle);
		if (JBPM.DocUtil.isFF) {
			JBPM.DocUtil.removeEventHandler(document, "mousemove",
					this._moveHandler);
			JBPM.DocUtil.removeEventHandler(document, "mouseup",
					this._endHandler);
		}
		if (JBPM.DocUtil.isIE) {
			JBPM.DocUtil.removeEventHandler(this.canvas, "mousemove",
					this._moveHandler);
			JBPM.DocUtil.removeEventHandler(this.canvas, "mouseup",
					this._endHandler);
		}
	},
	...
	moving : function(event) {
		...
		with (md.style) {
			...
			if (Math.abs(l) + JBPM.container.offsetWidth >= md.offsetWidth - 2
					* parseInt(JBPM.container.style.borderWidth)) {
			...
			if (Math.abs(t) + JBPM.container.offsetHeight >= md.offsetHeight
					- 2 * parseInt(JBPM.container.style.borderWidth)) {
	...
	drawArrow : function(startPoint, endPoint) {
		var points = JBPM.Pen.getArrowPoint(startPoint, endPoint, this.angle,
				this.arrowLength);
	...
	drawLineArrow : function(points) {
		if (points.length <= 1) {
			...
			this
					.drawArrow(points[points.length - 2],
...
JBPM.Pen = function(painter, obj) {
	this.init.apply(this, [ painter, obj ]);
};
		...
		if (Math.abs(x2 - x1) < 0.1 && y2 < y1) {
			...
			if (Math.abs(x2 - x1) < 0.1 && y2 > y1) {
				...
				if (x1 == x2) {
					...
					angle = Math.atan(parseFloat(y1 - y2) / (x1 - x2)) * 180
							/ Math.PI
				}
			}
		}
...
JBPM.Pen.prototype = {
	...
	bindEvent : function() {
		this._startHandle = JBPM.DocUtil.BindAsEventListener(this,
				this.startPen);
		this._moveHandler = JBPM.DocUtil
				.BindAsEventListener(this, this.movePen);
	...
	drawLine : function(startPoint, endPoint) {
		this.context.beginPath();
		if (startPoint == null || endPoint == null || startPoint.x == null
				|| startPoint.y == null || endPoint.x == null
				|| endPoint.y == null) {
			return;
		}
		this.context.moveTo(startPoint.x, startPoint.y);
		...
		if (this.downPoint == null) {
			...
			if (JBPM.DataBase.Nodes.length != 0) {
				for (var i = 0; i < JBPM.DataBase.Nodes.length; i++) {
					...
					if (((this.downPoint.x - JBPM.FlowArea.getLeft() - temp.offsetLeft)
							* (this.downPoint.x - JBPM.FlowArea.getLeft() - (temp.offsetLeft + temp.offsetWidth)) < 0)
							&& ((this.downPoint.y - JBPM.FlowArea.getTop() - temp.offsetTop)
									* (this.downPoint.y
											- JBPM.FlowArea.getTop() - (temp.offsetTop + temp.offsetHeight)) < 0)) {
						this.downPoint.x = parseInt(temp.offsetLeft
								+ temp.offsetWidth / 2);
						this.downPoint.y = parseInt(temp.offsetTop
								+ temp.offsetHeight / 2);
						this.startNode = temp;
	...
	isMouseUpOnNode : function() {
		if (this.upPoint == null) {
			...
			if (JBPM.DataBase.Nodes.length != 0) {
				for (var i = 0; i < JBPM.DataBase.Nodes.length; i++) {
					var temp = JBPM.DataBase.Nodes[i].HTMLObj;
					if (((this.upPoint.x - JBPM.FlowArea.getLeft() - temp.offsetLeft)
							* (this.upPoint.x - JBPM.FlowArea.getLeft() - (temp.offsetLeft + temp.offsetWidth)) < 0)
							&& ((this.upPoint.y - JBPM.FlowArea.getTop() - temp.offsetTop)
									* (this.upPoint.y - JBPM.FlowArea.getTop() - (temp.offsetTop + temp.offsetHeight)) < 0)) {
						this.upPoint.x = parseInt(temp.offsetLeft
								+ temp.offsetWidth / 2);
						this.upPoint.y = parseInt(temp.offsetTop
								+ temp.offsetHeight / 2);
						this.endNode = temp;
	...
	getJoint : function() {
		...
		this.downPoint = getXPoint(st, 'y', 'start')
				|| getXPoint(st + sh, 'y', 'start')
				|| getXPoint(sl, 'x', 'start')
				|| getXPoint(sl + sw, 'x', 'start');
		...
		this.upPoint = getXPoint(et, 'y', 'end')
				|| getXPoint(et + eh, 'y', 'end') || getXPoint(el, 'x', 'end')
				|| getXPoint(el + ew, 'x', 'end');
		function getXPoint(a, fx, nodeType) {
			...
			if (((xPoint.x - x2) * (xPoint.x - x1) <= 0)
					&& ((xPoint.y - y2) * (xPoint.y - y1) <= 0)) {
				if (nodeType == 'start') {
					if ((xPoint.x - sl) >= 0 && (xPoint.x - (sl + sw)) <= 0
							&& (xPoint.y - st) >= 0
							&& (xPoint.y - (st + sh)) <= 0) {
						return xPoint;
					}
				} else {
					if ((xPoint.x - el) >= 0 && (xPoint.x - (el + ew)) <= 0
							&& (xPoint.y - et) >= 0
							&& (xPoint.y - (et + eh)) <= 0) {
						return xPoint;
					}
...
JBPM.Util.resetMoveDiv = function(moveDiv) {
	...
	if (l + moveDiv.offsetWidth > JBPM.container.offsetWidth - 2
			* parseInt(JBPM.container.style.borderWidth)) {
		moveDiv.style.left = JBPM.container.offsetWidth - moveDiv.offsetWidth
				- 2 * parseInt(JBPM.container.style.borderWidth) + "px";
	}
...
JBPM.Util.checkNodesPosition = function() {
	...
	var jw = JBPM.container.offsetWidth - 2
			* parseInt(JBPM.container.style.borderWidth);
	var jh = JBPM.container.offsetHeight - 2
			* parseInt(JBPM.container.style.borderWidth);
...
JBPM.Util.initNodes = function(xml) {
	...
	for (var i = 0; i < nodes.length; i++) {
		...
		//add by SBJ  记录复选框是否选中
		for (var k = 0; k < newNode.properties.length; k++) {
		    var item = newNode.properties[k];
			var name = item.name;
			var value = nodes[i].getAttribute(name);
			 
			if(name =='nonMuiltAudit'|| name =='rollbackSing' ){
			    if(value==1){
				 var input = document.getElementsByName(name)[i-1];
				 input.checked=true;
				}
			}
			...
			//add by SBJ  
			if(name=='departmentModel'){
				   if(value==0){
				      var select = document.getElementsByName('department')[i-1];
					  select.parentNode.parentNode.style.display = '';
				   }else{
				      var select = document.getElementsByName('department')[i-1];
...
JBPM.Util.createXML = function() {
	...
	for (var i = 0; i < nodes.length; i++) {
		...
		//alert(JBPM.Util.getOuterHTML(nodeXML));// 单个元素组成XML add by chenck
...
JBPM.Util.getXpoints = function(startHTMLObj, endHTMLObj, isLabel) {
	...
	var downPoint = {};
	downPoint.x = parseInt(startHTMLObj.offsetLeft + startHTMLObj.offsetWidth
			/ 2);
	downPoint.y = parseInt(startHTMLObj.offsetTop + startHTMLObj.offsetHeight
			/ 2);
	...
	downPoint = getXPoint(st, 'y', 'start') || getXPoint(st + sh, 'y', 'start')
			|| getXPoint(sl, 'x', 'start') || getXPoint(sl + sw, 'x', 'start');
	result.startPoint = downPoint;
	upPoint = getXPoint(et, 'y', 'end') || getXPoint(et + eh, 'y', 'end')
			|| getXPoint(el, 'x', 'end') || getXPoint(el + ew, 'x', 'end');
	...
	function getXPoint(a, fx, nodeType) {
		...
		if (((xPoint.x - x2) * (xPoint.x - x1) <= 0)
			...
			if (nodeType == 'start') {
				if ((xPoint.x - sl) >= 0 && (xPoint.x - (sl + sw)) <= 0
						&& (xPoint.y - st) >= 0 && (xPoint.y - (st + sh)) <= 0) {
					return xPoint;
				}
			} else {
				if ((xPoint.x - el) >= 0 && (xPoint.x - (el + ew)) <= 0
						&& (xPoint.y - et) >= 0 && (xPoint.y - (et + eh)) <= 0) {
					return xPoint;
				}
			}
		}
...
JBPM.Util.getXpoint = function(HTMLObj, point) {
	...
	downPoint = getXPoint(st, 'y') || getXPoint(st + sh, 'y')
			|| getXPoint(sl, 'x') || getXPoint(sl + sw, 'x');
	return downPoint;
...
JBPM.Util.getDistancePointOnLine = function(spoint, epoint, mpoint) {
	...
	if (spoint.x - epoint.x != 0) {
		...
		return Math.abs(k * mpoint.x + (-1) * mpoint.y + b)
				/ (Math.sqrt(k * k + 1));
	} else {
...
JBPM.ToolBox.prototype = {
	init : function(obj) {
		...
		if (this.movable) {
			...
			move.appendChild(document.createTextNode("工具栏"));
		...
		tool = new JBPM.ToolBox.Tool({
			...
			properties : [{
				...
				label : '指定部门',
				type : 'select',
				selectValues : JBPM.DictUtil.Department(),
				defaultValue : ''
			}, {
				name : 'station',
				label : '指定岗位',
				type : 'checkbox_div',
				selectValues : JBPM.DictUtil.Station(),
				defaultValue : ''
			},
			{
				name : 'viewButtons',
				label : '按钮设置',
				type : 'checkbox_div2',
				selectValues:JBPM.DictUtil.viewButtons(),
				defaultValue : ''
			},{
				name : 'clickButtons',
				label : '按钮必点设置',
				type : 'checkbox_div2',
				selectValues:JBPM.DictUtil.clickButtons(),
				defaultValue : ''
			},{
				name : 'nonMuiltAudit',
				label : '回退无需审核',
				...
				name : 'rollbackEditAtomics',
				label : '回退可编辑原子',
				type : 'checkbox_div',
				selectValues:JBPM.DictUtil.basicAtomics(),
				defaultValue : ''
			}
			,{
				name : 'viewAtomics',
			...
			,{
				name : 'editablebuscodes',
				label : '事中可修改原子',
				type : 'checkbox_div2',
				selectValues:JBPM.DictUtil.enEditBusCodes(),
				defaultValue : ''
			},{
				name : 'excuteInMatterBusCodes',
				label : '事中可执行原子',
				type : 'checkbox_div2',
				selectValues:JBPM.DictUtil.excuteInMatterBusCodes(),
				defaultValue : ''
			},{
				name : 'desc',
				label : '备注',
...
//				}
	...
	getWidth : function() {
		return this.container.firstChild.offsetWidth == 0 ? 100
				: this.container.firstChild.offsetWidth;
	},
...
JBPM.ToolBox.Tool = function(obj) {
	this.init.apply(this, [ obj ]);
};
JBPM.ToolBox.Tool.prototype = {
	init : function(obj) {
		...
		this._clickHandle = JBPM.DocUtil.BindAsEventListener(this,
				this.clickHandler);
		this._deleteHandle = JBPM.DocUtil.BindAsEventListener(this,
				this.removeMoveDiv);
		this._changeBgColorHandlle = JBPM.DocUtil.BindAsEventListener(this,
				this.activeHandler);
		if (obj.clickHandler) {
			this._clickHandle = JBPM.DocUtil.BindAsEventListener(this,
					obj.clickHandler);
	...
	createMoveDiv : function(obj, e) {
		...
		if (obj) {
			...
			if (e) {
				moveDiv.style.left = parseInt(JBPM.getX(e)
						- moveDiv.offsetWidth / 2)
						+ 'px';
				moveDiv.style.top = parseInt(JBPM.getY(e)
	...
	createHTML : function() {
		...
		JBPM.DocUtil.addEventHandler(this.li, "mousedown",
				this._changeBgColorHandlle);
	...
	moving : function(event) {
		...
		JBPM.pen.clearLine();
		JBPM.pen.show();
		var startNodeRelations = JBPM.DataBase.NodeRelations
				.getStartNodeRelations(md.id);
		var endNodeRelations = JBPM.DataBase.NodeRelations
				.getEndNodeRelations(md.id);
			...
			for (var i = 0; i < startNodeRelations.length; i++) {
				...
				if (JBPM.pen.animat) {
					if (sr.isStraightLine()) {
						var newPoint = JBPM.Util.getXpoints(
								sr.getStartNode().HTMLObj, md);
						JBPM.pen.drawArrow(newPoint.startPoint,
								newPoint.endPoint);
					} else {
			...
			for (var i = 0; i < endNodeRelations.length; i++) {
				...
				if (JBPM.pen.animat) {
					if (er.isStraightLine()) {
						var newPoint = JBPM.Util.getXpoints(md,
								er.getEndNode().HTMLObj);
						JBPM.pen.drawArrow(newPoint.startPoint,
								newPoint.endPoint);
					} else {
	...
	endMove : function(event) {
		...
		if (md != null) {
			...
			if (md.parentNode == JBPM.container) {
				md.style.left = parseInt(md.style.left)
						- JBPM.Util.getInitLeft(JBPM.FlowArea) + "px";
				md.style.top = parseInt(md.style.top)
						- JBPM.Util.getInitTop(JBPM.FlowArea) + "px";
	...
	initEvent : function(moveDiv) {
		...
		this._moveHandler = JBPM.DocUtil.BindAsEventListener(this, this.moving);
		this._endHandler = JBPM.DocUtil.BindAsEventListener(this, this.endMove);
		this._overHandler = JBPM.DocUtil.BindAsEventListener(this,
				this.showDeleteBtn);
		this._outHandler = JBPM.DocUtil.BindAsEventListener(this,
	...
	removeEvent : function() {
		JBPM.DocUtil.removeEventHandler(this.getMoveDiv(), "mousedown",
				this._startHandle);
		JBPM.DocUtil.removeEventHandler(document, "mousemove",
				this._moveHandler);
		if (JBPM.DocUtil.isFF) {
		...
		if (JBPM.DocUtil.isIE) {
			JBPM.DocUtil.removeEventHandler(this.getMoveDiv(), "mouseup",
					this._endHandler);
		}
	},
...
JBPM.Point.prototype = {
	...
	initEvent : function() {
		...
		this._moveHandler = JBPM.DocUtil.BindAsEventListener(this, this.moving);
		this._endHandler = JBPM.DocUtil.BindAsEventListener(this, this.endMove);
		JBPM.DocUtil.addEventHandler(this.HTMLObj, "mousedown",
				this._startHandle);
		JBPM.DocUtil.addEventHandler(document, "mousemove", this._moveHandler);
		if (JBPM.DocUtil.isFF) {
			JBPM.DocUtil.addEventHandler(document, "mouseup", this._endHandler);
		...
		if (JBPM.DocUtil.isIE) {
			JBPM.DocUtil.addEventHandler(this.HTMLObj, "mouseup",
					this._endHandler);
		}
	},
	removeEvent : function() {
		JBPM.DocUtil.removeEventHandler(this.HTMLObj, "mousedown",
				this._startHandle);
		JBPM.DocUtil.removeEventHandler(document, "mousemove",
				this._moveHandler);
		if (JBPM.DocUtil.isFF) {
		...
		if (JBPM.DocUtil.isIE) {
			JBPM.DocUtil.removeEventHandler(this.HTMLObj, "mouseup",
					this._endHandler);
		}
	},
	...
	getStartPoint : function() {
		...
		if (!this.isSpecialPoint) {
			if (this.isStartPoint()) {
				if (this.isEndPoint()) {
					var temp = JBPM.Util.getXpoints(
							this.relation.startNode.HTMLObj,
							this.relation.endNode.HTMLObj);
					sp = temp.startPoint;
				} else {
					sp = JBPM.Util.getXpoint(this.relation.startNode.HTMLObj,
							this.nextPoint.getPosition());
				}
			} else {
		...
		if (!this.isSpecialPoint) {
			if (this.isEndPoint()) {
				if (this.isStartPoint()) {
					...
					ep = JBPM.Util.getXpoint(this.relation.endNode.HTMLObj,
							this.lastPoint.getPosition());
				}
			} else {
	...
	getLastSpecialPoint : function() {
		...
		if (this.lastPoint.isStartPoint()) {
			return JBPM.Util.getXpoint(this.relation.startNode.HTMLObj, this
					.getPosition());
		} else {
	...
	getNextSpecialPoint : function() {
		...
		if (this.nextPoint.isEndPoint()) {
			return JBPM.Util.getXpoint(this.relation.endNode.HTMLObj, this
					.getPosition());
		} else {
	...
	getStartMovePoint : function() {
		if (!this.isStartPoint()) {
			...
			var temp = JBPM.Util.getXpoint(this.relation.startNode.HTMLObj,
					this.getPosition());
	...
	getEndMovePoint : function() {
		if (!this.isEndPoint()) {
			...
			var temp = JBPM.Util.getXpoint(this.relation.endNode.HTMLObj, this
					.getPosition());
	...
	resetSidePoints : function() {
		...
		this.lastPoint.setPosition(JBPM.Util.getMiddlePoint(s, this
				.getPosition()));
		this.nextPoint.setPosition(JBPM.Util.getMiddlePoint(e, this
				.getPosition()));
	},
...
JBPM.DataBase.Base = function(obj) {
	this.init.apply(this, [ obj ]);
};
JBPM.DataBase.Base.prototype = {
	...
	createPropertiesTable : function() {
		...
		if (this.properties) {
			...
			this.propertiesTable.style.width ="100%";
			...
			for (var i = 0; i < this.properties.length; i++) {
				...
				this._blurHandler = JBPM.DocUtil.BindAsEventListener(this,
						this.changeProperty);
				var type = temp.type;
				if (type == 'select' && temp.selectValues) {
					...
					JBPM.DocUtil.addEventHandler(select, "change",
							this._blurHandler);
					td1.appendChild(select);
					//TODO
				} else if (type == 'multiselect' && temp.selectValues) {// 多选下拉框add by chenck 20150423
					...
					for (var j = 0; j < temp.selectValues.length; j++) {
						var option = document.createElement("option");
						option.value = temp.selectValues[j].value;
						for(var k=0; k<arr.length; k++){// 多选赋值
							 if (temp.selectValues[j].value == arr[k]) {
								option.selected = true;
							}
						}
						option.innerHTML = temp.selectValues[j].label;
					...
					span.innerHTML = temp.label;

					JBPM.DocUtil.addEventHandler(input, "click",
							this._blurHandler);
					td1.appendChild(input);
					td1.appendChild(span);
				} else if (type == 'checkbox_div' && temp.selectValues) {// 多个选项，模拟可多选下拉框 样式 add by chenck 
					//update by SBJ
					var arr = defaultValue.split(',');
					var div = document.createElement("div");
					div.style.height = "100px";// 高度
					div.style.overflow = "auto";// 滚动条
					div.style.border = "1px #000000 solid";//设定边框
					div.style.padding = "5px";//内部空白
					div.id = generateCode("div"); //自动生成ID
					for (var j = 0; j < temp.selectValues.length; j++) {
						var input = document.createElement("input");
						input.type = 'checkbox';
						input.name = name;
						...
						input.value = temp.selectValues[j].value;
						for(var k=0; k<arr.length; k++){// 多选赋值
							if (temp.selectValues[j].value == arr[k]) {
								input.checked = true;
							}
						}
						JBPM.DocUtil.addEventHandler(input, "click", this._blurHandler);
						var span = document.createElement("span");
						span.style.paddingLeft = "5px"; 
						span.innerHTML = temp.selectValues[j].label;
					...
					//alert(div.outerHTML);
					td1.appendChild(div);
				} else if (type == 'checkbox_div2' && temp.selectValues) {// 多个选项，模拟可多选下拉框 样式 add by zqg 
					//update by SBJ
					var arr = defaultValue.split(',');
					var div = document.createElement("div");
					div.style.height = "40px";// 高度
					div.style.overflow = "auto";// 滚动条
					div.style.border = "1px #000000 solid";//设定边框
					div.style.padding = "5px";//内部空白
					div.id = generateCode("div"); //自动生成ID
					...
					JBPM.DocUtil.addEventHandler(input, "keyup",
							this._blurHandler);
					td1.appendChild(input);
				}
				tr.appendChild(td1);
	...
	createXML : function() {
		var type = this.type.toLowerCase() == "script" ? "script-temp"
				: this.type;
		...
		for (var i = 0; i < this.properties.length; i++) {
			...
			if (temp.defaultValue) {
				if (temp.isChild) {
					...
					//alert(name + " - " + temp.defaultValue);
					this.xmlNode.setAttribute(name, temp.defaultValue);
				}
			}
		}
		this.oncreateXML();
		//alert("type="+type+","+"station=" + this.xmlNode.getAttribute("station"));
		//alert(this.xmlNode.outerHTML);
		return this.xmlNode;
	}
};
JBPM.DataBase.Process = function() {
	this.init.apply(this, [ {} ]);
};
...
JBPM.DataBase.Node = function(obj) {
	this.init.apply(this, [ obj ]);
};
JBPM.DataBase.Node.prototype = (new JBPM.DataBase.Base).extend({
	...
	changeProperty : function(event) {
		...
		if (obj != null) {
			...
			if (obj.type == 'checkbox') {
				if (obj.checked) {
					this.setValue(obj.name, 1);
				} else {
					this.setValue(obj.name, "");
				}
				//alert(this.getValue(obj.name));
				//update by SBJ
				if("station" == obj.name||"editablebuscodes" == obj.name||
						"excuteInMatterBusCodes" == obj.name||"viewButtons"==obj.name||
						"clickButtons"==obj.name||"rollbackEditAtomics"==obj.name||"viewAtomics"==obj.name){
					...
					if("" != checkedStr){
						var splitChar = checkedStr.substring(checkedStr.length-1);
						if("," == splitChar){
					...
					//alert(checkedStr);
					this.setValue(obj.name, checkedStr);
				}
			}else if (obj.type == 'select-multiple') {// 多选下拉框
				// 根据选择的元素赋值到数据模型中，以便生成XML传递到后台 add by chenck 20150423
				//alert(obj.name + "-" +$("#"+obj.name).val());
				this.setValue(obj.name, $("#"+obj.name).val());
			}else{
				this.setValue(obj.name, obj.value);
			}
			//add by SBJ
			if(obj.name=="departmentModel"){
				if(obj.value!=0){
					//所属table的id
					  var tabid = obj.parentNode.parentNode.parentNode.parentNode.id; 
					  var selects = document.getElementsByName('department');
					  for (var i = 0; i < selects.length; i++) {
					      var select = document.getElementsByName('department')[i];
					      var parentId = select.parentNode.parentNode.parentNode.parentNode.id;
					      if(parentId == tabid){
					  ...
					  this.setValue('department', "");
						  ...
						  for (var i = 0; i < selects.length; i++) {
						      ...
						      if(parentId == tabid){
							     ...
							     select.parentNode.parentNode.style.display = '';
							     this.setValue('department', select.value);
						      }
						   }
						}
			}
			...
			if (obj.name == 'name' && this.isNormal) {
				...
				JBPM.Util.resetInitPosition(this.HTMLObj);
				var startNodeRelations = JBPM.DataBase.NodeRelations
						.getStartNodeRelations(this.HTMLObj.id);
				var endNodeRelations = JBPM.DataBase.NodeRelations
						.getEndNodeRelations(this.HTMLObj.id);
				if (startNodeRelations && endNodeRelations) {
					for (var i = 0; i < startNodeRelations.length; i++) {
						var sr = startNodeRelations[i];
	...
	oncreateXML : function() {
		var gValue = this.HTMLObj.offsetLeft + "," + this.HTMLObj.offsetTop
				+ "," + this.HTMLObj.offsetWidth + ","
				+ this.HTMLObj.offsetHeight;
...
JBPM.DataBase.NodeRelation = function(obj) {
	this.init.apply(this, [ obj ]);
};
JBPM.DataBase.NodeRelation.prototype = (new JBPM.DataBase.Base)
		.extend({
			oninit : function(obj) {
				...
				this.properties = JBPM.Util.cloneArray(JBPM.NodeToolMap
				...
				this.labelRelativePoint = obj.labelRelativePoint ? obj.labelRelativePoint
						: {
							x : 0,
							y : 0
						};
			...
			clearPoints : function() {
				for (var i = 0; i < this.points.length; i++) {
					this.points[i].HTMLObj.parentNode
							.removeChild(this.points[i].HTMLObj);
				}
				this.points = [];
			},
			getCenterPoint : function() {
				...
				if (sps.length == 0) {
					var temp = JBPM.Util.getXpoints(this.startNode.HTMLObj,
							this.endNode.HTMLObj);
					return JBPM.Util.getMiddlePoint(temp.startPoint,
							temp.endPoint);
				} else {
					if (sps.length % 2 == 0) {
			...
			getLinePoints : function() {
				...
				var sps = this.getSpecialPoints();
				if (sps.length > 0) {
					var startPoint = JBPM.Util.getXpoint(
							this.startNode.HTMLObj, sps[0]);
					points.push(startPoint);
					points = points.concat(sps);
					var endPoint = JBPM.Util.getXpoint(this.endNode.HTMLObj,
							sps[sps.length - 1]);
					points.push(endPoint);
				} else {
					var sePoint = JBPM.Util.getXpoints(this.startNode.HTMLObj,
							this.endNode.HTMLObj);
			...
			initEvent : function(moveLabel) {
				this._startHandle = JBPM.DocUtil.BindAsEventListener(this,
						this.startMove);
				this._moveHandler = JBPM.DocUtil.BindAsEventListener(this,
						this.moving);
				this._endHandler = JBPM.DocUtil.BindAsEventListener(this,
						this.endMove);
				this._overHandler = JBPM.DocUtil.BindAsEventListener(this,
						this.showDeleteBtn);
				this._outHandler = JBPM.DocUtil.BindAsEventListener(this,
				...
				JBPM.DocUtil.addEventHandler(moveLabel, "mousedown",
						this._startHandle);
				JBPM.DocUtil.addEventHandler(moveLabel, "mouseover",
						this._overHandler);
				JBPM.DocUtil.addEventHandler(moveLabel, "mouseout",
						this._outHandler);
				JBPM.DocUtil.addEventHandler(document, "mousemove",
						this._moveHandler);
				if (JBPM.DocUtil.isFF) {
					JBPM.DocUtil.addEventHandler(document, "mouseup",
							this._endHandler);
				}
				if (JBPM.DocUtil.isIE) {
					JBPM.DocUtil.addEventHandler(this.label, "mouseup",
							this._endHandler);
				}
			},
			startMove : function(event) {
			...
			endMove : function(event) {
				...
				this.labelRelativePoint = {
					x : parseInt(this.label.style.left) - mp.x,
					y : parseInt(this.label.style.top) - mp.y
				};
				JBPM.pen.hidden();
			},
			removeEvent : function() {
				JBPM.DocUtil.removeEventHandler(this.label, "mousedown",
						this._startHandle);
				JBPM.DocUtil.removeEventHandler(document, "mousemove",
						this._moveHandler);
				if (JBPM.DocUtil.isFF) {
				...
				if (JBPM.DocUtil.isIE) {
					JBPM.DocUtil.removeEventHandler(this.label, "mouseup",
							this._endHandler);
				}
			},
			getLabel : function() {
			...
			isPointOnRelation : function(point) {
				...
				for (var i = 1; i < sps.length; i++) {
					var t = JBPM.Util.getDistancePointOnLine(sps[i - 1],
							sps[i], point);
					if (t > 0 && t < 5) {
						return true;
					}
			...
			createLabel : function() {
				...
				with (this.label.style) {
					...
					height = 'auto';
					fontSize = '12px';
					left = parseInt(lp.x) + parseInt(this.labelRelativePoint.x)
							+ 'px';
					top = parseInt(lp.y) + parseInt(this.labelRelativePoint.y)
							+ 'px';
					whiteSpace = "nowrap";
					zIndex = 2;
				}
			...
			showLabel : function() {
				if (this.label == null) {
					...
					if (lp != null && lp.x != null && lp.y != null) {
						with (this.label.style) {
							left = parseInt(lp.x)
									+ parseInt(this.labelRelativePoint.x)
									+ 'px';
							top = parseInt(lp.y)
									+ parseInt(this.labelRelativePoint.y)
									+ 'px';
						}
					}
					JBPM.Util.resetInitPosition(this.label);
			...
			createInitPoint : function() {
				if (this.initSpecialPoints.length == 0) {
					...
					for (var i = 0; i < this.initSpecialPoints.length; i++) {
						var mpoint = new JBPM.Point({
							startPoint : i == 0 ? this.startPoint
									: this.initSpecialPoints[i - 1],
						...
						var spoint = new JBPM.Point(
								{
									startPoint : i == 0 ? this.startPoint
											: this.initSpecialPoints[i - 1],
									endPoint : i == (this.initSpecialPoints.length - 1) ? this.endPoint
											: this.initSpecialPoints[i + 1],
									isStartPoint : i == 0 ? true : false,
									isSpecialPoint : true,
									isEndPoint : i == (this.initSpecialPoints.length - 1) ? true
											: false,
...
JBPM.DataBase.NodeRelations.isExist = function(rela) {
	for (var i = 0; i < JBPM.DataBase.NodeRelations.length; i++) {
		...
		if (ral.endNodeId == rela.endNodeId
				&& ral.startNodeId == rela.startNodeId) {
			return true;
		}
	}
	return false;
...
JBPM.Image = {};
// 重新加载图片
JBPM.loadImages = function reload() {
...
JBPM.Text.modeLabel = "工作模式";
JBPM.Text.selectMode = "选择|拖拽";
JBPM.Text.transitionMode = "连接模式";
