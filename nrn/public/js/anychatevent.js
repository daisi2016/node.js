...
function OnAnyChatRecordSnapShot(dwUserId, lpFileName, dwParam, bRecordType) {
 alert("record local path:"+lpFileName);
 FILEPATH =lpFileName;
...
function OnAnyChatEnterRoom(dwRoomId, errorcode) {
    ...
    if (errorcode == 0) {
		...
		ShowNotifyMessage("您已成功进入" + dwRoomId + "号房间,请录制并上传见证视频。", NOTIFY_TYPE_SYSTEM);
        // 设置本地视频显示位置
...
function OnAnyChatCameraStateChange(dwUserId, State) {
	if(GetID(dwUserId + "_CameraTag") != null){
		 ...
		 if (State == 1) GetID(dwUserId + "_CameraTag").src = globalRoot + "/themes/default/images/anychat/advanceset/camera_false.png";
		 if (State == 2) GetID(dwUserId + "_CameraTag").src = globalRoot + "/themes/default/images/anychat/advanceset/camera_true.png";
	}
}
